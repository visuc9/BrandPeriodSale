(function () {
    var commands = {
        toolbarFocus: {
            ...
            exec: function (editor) {
                var idBase = editor._.elementsPath_custom.idBase;
    ...
    var emptyHtml = '<span class="cke_path_empty_custom">&nbsp;</span>';
    ...
    CKEDITOR.plugins.add('elementspath_custom', {
        ...
        init: function (editor) {
            editor._.elementsPath_custom = {
                idBase: 'cke_elementspath_custom_' + CKEDITOR.tools.getNextNumber() + '_',
                filters: []
            };
    ...
    function initElementsPath(editor, bottomSpaceData) {
        var spaceId = editor.ui.spaceId('path'),
			...
			elementsPath_custom = editor._.elementsPath_custom,
			idBase = elementsPath_custom.idBase;

        // CUSTOM: Remove
        bottomSpaceData.html += '<span id="' + spaceId + '_label" class="cke_voice_label">' + editor.lang.elementspath.eleLabel + '</span>' +
			'<span id="' + spaceId + '" class="cke_pathX" role="group" aria-labelledby="' + spaceId + '_label">' + emptyHtml + '</span>';
        //bottomSpaceData.html = ""; //"<span style='color: blue'>XXXX</span>";
        ...
        function onClick(elementIndex) {
            var element = elementsPath_custom.list[elementIndex],
				selection;
			...
			onKeyDownHandler = CKEDITOR.tools.addFunction(function (elementIndex, ev) {
			    ...
			    switch (ev.getKeystroke()) {
			        ...
			        case CKEDITOR.SHIFT + 9: // SHIFT + TAB
			            element = CKEDITOR.document.getById(idBase + (elementIndex - 1));
			            if (!element)
			                element = CKEDITOR.document.getById(idBase + (elementsPath_custom.list.length - 1));
			            element.focus();
			            return false;
        ...
        editor.on('selectionChange', function () {
            var html = [],
				elementsList = elementsPath_custom.list = [],
				namesList = [],
				filters = elementsPath_custom.filters,
        ...
        function empty() {
            ...
            delete elementsPath_custom.list;
        }
...
/**
 * Fired when the contents of the elementsPath_custom are changed.
 *
